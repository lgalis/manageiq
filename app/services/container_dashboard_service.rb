class ContainerDashboardService
  ...
  def hourly_network_metrics
    ...
    options = { :conditions => ['resource_type = ? AND resource_id in (?)', 'ExtManagementSystem', resource_ids]}

    hourly_metrics = MetricRollup.find_all_by_interval_and_time_range(
      "hourly",
      1.day.ago.beginning_of_hour.utc,
      Time.now.utc,
      :all,
      options)
